...
#### 页面级
- **字段：功能入口** - 类型：按钮；描述：我发起的任务中，人员清单行的"是否参与"按钮
- **前置条件**：任务状态=已接单（专家已接单）
- **后置影响**：更新专家的参与状态和实际工作周期

#### 弹窗级
- **弹窗：ParticipateConfirmDialog**
  - **触发入口**：点击"是否参与"按钮
  - **关闭方式**：关闭图标 / 取消按钮
  - **前置条件**：任务已有人接单（状态=已接单）
  - **字段列表**：
    | 字段名 | 类型 | 必填 | 默认值 | 来源 | 校验规则 | 展示形式 | 交互约束 |
    |--------|------|------|--------|------|----------|----------|----------|
    | 专家姓名 | 文本 | 是 | - | 任务数据（当前人员姓名） | - | 文字 | 只读 |
    | 是否参与 | 单选 | 是 | 是 | 用户选择 | - | RadioGroup（是/否） | 可编辑 |
    | 开始时间 | 日期 | 条件 | 空 | 用户选择 | 非空（选"是"时必填） | 日期选择器 | 可编辑（选"是"时激活） |
    | 结束时间 | 日期 | 条件 | 空 | 用户选择 | 非空（选"是"时必填），结束时间≥开始时间 | 日期选择器 | 可编辑（选"是"时激活） |
  - **联动逻辑**：
    - 选择"否"时：开始时间、结束时间字段置灰不可用，不做校验
    - 选择"是"时：开始时间、结束时间字段激活，必填校验生效
  - **确定按钮**：调用 `POST /api/task/confirm-participate` 接口，提交参与确认
  - **取消按钮**：关闭弹窗，不调用接口，不修改数据
...
interface TaskMember {
  ...
  actualStartTime?: string;     // 实际开始时间（参与确认时填写）
  actualEndTime?: string;       // 实际结束时间（参与确认时填写）
  participate?: boolean;        // 是否参与（null=未确认，true=参与，false=不参与）
  reports?: Report[];           // 回单记录
}
...
### 接口9：确认参与（是否参与）
- **请求路径**：`POST /api/task/confirm-participate`
- **请求方法**：POST
- **请求参数**：
  | 参数名 | 类型 | 必填 | 来源 | 说明 |
  |--------|------|------|------|------|
  | taskId | string | 是 | 任务ID | 任务ID |
  | memberId | string | 是 | 成员ID | 专家人员ID |
  | participate | boolean | 是 | 用户选择 | 是否参与（true=是，false=否） |
  | startTime | string | 条件 | 用户选择 | 开始时间（选"是"时必填） |
  | endTime | string | 条件 | 用户选择 | 结束时间（选"是"时必填） |
- **响应**：
  ```json
  { "success": true }
  ```
- **存储位置**：数据库表 `task_member`（actualStartTime、actualEndTime 字段）
- **错误码**：
  - `400` - 参数校验失败（选"是"但时间未填，或结束时间早于开始时间）
  - `403` - 无权限操作该成员
  - `404` - 任务或成员不存在

### 数据刷新点
- **刷新时机**：发布/审核/派单/申请/接单/拒绝/撤单/补充/确认参与操作成功后
- **影响字段**：任务列表、人员清单中的参与状态
...
- [ ] **操作**：发起人点击"是否参与"按钮 → **预期**：弹出参与确认弹窗，显示专家姓名、是否参与单选、开始/结束时间
- [ ] **操作**：选择"是"，填写开始/结束时间后点击确定 → **预期**：接口调用成功，弹窗关闭，专家工作时间已记录
- [ ] **操作**：选择"否"后点击确定 → **预期**：接口调用成功，弹窗关闭，记录专家不参与
...
- [ ] **操作**：选择"是"但未填写时间直接提交 → **预期**：提示"开始时间和结束时间不能为空"
- [ ] **操作**：选择"是"后结束时间早于开始时间 → **预期**：提示"结束时间必须大于开始时间"
...
### v7 - 2026-05-22
- 状态流转图增加"发起人确认是否参与"状态
- 新增"人员参与状态流转图"，展示已接单后专家的参与确认流程

### v6 - 2026-05-22
- 新增功能点13：专家参与确认弹窗（ParticipateConfirmDialog）
- 新增接口9：确认参与（POST /api/task/confirm-participate）
- 补充参与确认的联动逻辑：选择"是"时时间字段激活，"否"时置灰
- 补充验收标准：参与确认正常流程和异常流程
